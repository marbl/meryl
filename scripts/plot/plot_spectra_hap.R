...
parser$add_argument("-m", "--max", type="integer", default=100, help="maximum limit for k-mer multiplicity [default %(default)s]")
args <- parser$parse_args()

spectra_hap_plot  <-  function(hist, name, w=6, h=3, x_max=100) {
  
  dat=read.table(hist, header = TRUE)
  y_max=max(dat[dat[,1]!="read-total",]$Count)
  y_max=y_max*1.5
  ...
  ## Line graph
  ggplot(data=dat, aes(x=kmer_multiplicity, y=Count, group=Assembly, colour=Assembly)) +
    geom_line() +
    theme_bw() +
  ...
  ggplot(data=dat, aes(x=kmer_multiplicity, y=Count)) +
    geom_ribbon(aes(ymin=0, ymax=pmax(Count,0), fill=Assembly, colour=Assembly), alpha=0.5, linetype=1) +
    theme_bw() +
