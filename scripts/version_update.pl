...
elsif ($cwd =~ m/meryl-(.{40})\/src/) {
    $label    = "snapshot";
    $hash1    = $1;
    $hash2    = $1;
}

elsif ($cwd =~ m/meryl-master\/src/) {
    $label    = "master-snapshot";
}

elsif ($cwd =~ m/meryl-(\d).(\d)\/src/) {
    $label    = "release";
    $major    = $1;
    $minor    = $2;
}
...
open(F, "> meryl_version.H.new") or die "Failed to open 'meryl-version.H.new' for writing: $!\n";
print F "//  Automagically generated by meryl_version_update.pl!  Do not commit!\n";
print F "#define MERYL_VERSION_LABEL     \"$label\"\n";
print F "#define MERYL_VERSION_MAJOR     \"$major\"\n";
print F "#define MERYL_VERSION_MINOR     \"$minor\"\n";
print F "#define MERYL_VERSION_COMMITS   \"$commits\"\n";
print F "#define MERYL_VERSION_REVISION  \"$revCount\"\n";
print F "#define MERYL_VERSION_HASH      \"$hash1\"\n";

if      (defined($dirty)) {
    print F "#define MERYL_VERSION           \"Meryl snapshot v$major.$minor +$commits changes (r$revCount $hash1)\\n\"\n";
} elsif (defined($hash1)) {
    print F "#define MERYL_VERSION           \"Meryl snapshot ($hash1)\\n\"\n";
} else {
    print F "#define MERYL_VERSION           \"Meryl $major.$minor\\n\"\n";
}
...
if (compare("meryl_version.H", "meryl_version.H.new") == 0) {
    unlink "meryl_version.H.new";
} else {
    unlink "meryl_version.H";
    rename "meryl_version.H.new", "meryl_version.H";
}
