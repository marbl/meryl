...
echo -e "\t[out-prefix]: output prefix. Only used when asm2.fasta is provided."
echo
...
# 0-counts in the asm; only seen in the read reads
...
echo "Plot $name.spectra-cn.hist"
...
ERROR=`echo "$ASM_ONLY $TOTAL" | awk -v k=$k '{print (1-(-1/k)*log(1-$1/$2))}'`
QV=`echo "$ASM_ONLY $TOTAL" | awk -v k=$k '{print (-10*log((-1/k)*log(1-$1/$2))/log(10))}'`
echo -e "Both\t$ASM_ONLY\t$TOTAL\t$QV\t$ERROR" >> $name.qv
rm -r ${asm1}_or_${asm2}.0.meryl ${asm1}_${asm2}_union.meryl

echo "Generate spectra-hap.hist for combined $asm1_fa and $asm2_fa"
